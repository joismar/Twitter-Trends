...
def make_sentiment(value):
    ...
    if value == None or (value<-1 or value>1):
        return None
    else:
        return value
...
def analyze_tweet_sentiment(tweet):
    ...
    sentiment_count = 0
    sentiment_sum = 0
    for sentiment in tweet_words(tweet):
        if has_sentiment(get_word_sentiment(sentiment)):
            a = get_word_sentiment(sentiment)
            sentiment_sum += a
            sentiment_count += 1
...
def find_centroid(polygon):
    ...
    for a in range(len(polygon)) :
        ...
        if a != (len(polygon)-1) :
            sum1 += (polygon[a][0] * polygon[a+1][1]) - (polygon[a][1] * polygon[a+1][0])
        else :
            sum1 += (polygon[a][0] * polygon[0][1]) - (polygon[a][1] * polygon[0][0])
    area = sum1/2
    ...
    for a in range(len(polygon)) :
        ...
        if a != (len(polygon)-1) :
            sum1 += (polygon[a][0] + polygon[a+1][0]) * ((polygon[a][0] * polygon[a+1][1]) - (polygon[a][1] * polygon[a+1][0]))
        else :
            sum1 += (polygon[a][0] + polygon[0][0]) * ((polygon[a][0] * polygon[0][1]) - (polygon[a][1] * polygon[0][0]))
    centroidx = sum1/(6*area)

    sum1 = 0
    for a in range(len(polygon)) :
        
        if a != (len(polygon)-1) :
            sum1 += (polygon[a][1] + polygon[a+1][1]) * ((polygon[a][0] * polygon[a+1][1]) - (polygon[a][1] * polygon[a+1][0]))
        else :
            sum1 += (polygon[a][1] + polygon[0][1]) * ((polygon[a][0] * polygon[0][1]) - (polygon[a][1] * polygon[0][0]))
    centroidy = sum1/(6*area)
        
    return (centroidx, centroidy, abs(area))
...
def find_center(polygons):
    ...
    for polygon in polygons :
        sum1x += find_centroid(polygon)[0] * find_centroid(polygon)[2]
    for polygon in polygons :
        sum1y += find_centroid(polygon)[1] * find_centroid(polygon)[2]

    for polygon in polygons :
        sum2 += find_centroid(polygon)[2]

    return (sum1x/sum2, sum1y/sum2)
...
def find_closest_state(tweet, state_centers):
    ...
    'NJ'
    """
    temp = state_centers.copy()

    for a in temp :
        temp[a] = geo_distance(temp[a],tweet_location(tweet))
...
def group_tweets_by_state(tweets):
    ...
    for a in tweets :
        if not(find_closest_state(a, us_centers) in tweets_by_state) :
            tweets_by_state[find_closest_state(a, us_centers)] = [a]
        elif tweets_by_state[find_closest_state(a, us_centers)] == [a] :
            tweets_by_state[find_closest_state(a, us_centers)].append(a)
        else :
            tweets_by_state[find_closest_state(a, us_centers)].append(a)
...
def most_talkative_state(term):
    ...
    for a in tweets :
        state = find_closest_state(a, us_centers)
        if state in term_count :
            term_count[state] = term_count[state]+1
        else :
            term_count[state] = 1

    return max(term_count, key=term_count.get)
    ...
    for state in tweets_by_state :
        ...
        for tweet in tweets_by_state[state] :
            if analyze_tweet_sentiment(tweet) != None :
                a = analyze_tweet_sentiment(tweet)
                average_sum += a
                average_count += 1
        if average_count != 0 :
...
def group_tweets_by_hour(tweets):
    ...
    for tweet in tweets :
        if not(tweet_time(tweet).hour in tweets_by_hour) :
            tweets_by_hour[tweet_time(tweet).hour] = [tweet]
        elif tweets_by_hour[tweet_time(tweet).hour] == [tweet] :
